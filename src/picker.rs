...
//!
//! This replaces `select_image()` (the rofi call) from the bash script.
//! Everything downstream -- thumbnail cache, awww invocation, namespaces --
//! is untouched; this module's only job is to show the cached thumbnails and
//! return the filename the user clicked (or `None` on Escape/close).
//!
...
impl Picker {
    ...
    fn move_kbd_cursor(&mut self, delta: i32) {
        ...
        let new_idx = (self.kbd_index as i32 + delta).clamp(0, self.filtered.len() as i32 - 1) as usize;
    ...
    fn render_content(&self, content: &mut [u8]) {
        ...
        // Background: window-rounded panel, split into a search-bar band
        // and the icon area below it so they can have different shades.
        let bg = t.background.0;
        let bg_bgra = [bg[2], bg[1], bg[0], bg[3]];
        let sbar = t.search_bar_background.0;
        let sbar_bgra = [sbar[2], sbar[1], sbar[0], sbar[3]];
        for y in 0..h {
            let row_color = if y < t.search_bar_height { sbar_bgra } else { bg_bgra };
            for x in 0..w {
                let di = ((y * w + x) * 4) as usize;
                if corner_mask(x, y, w, h, t.window_corner_radius) {
                    content[di..di + 4].copy_from_slice(&[0, 0, 0, 0]); // transparent outside rounded panel
                } else {
                    content[di..di + 4].copy_from_slice(&row_color);
                }
            }
        }

        // Search text, left-aligned with a small cursor bar after it.
        let text_color = t.search_text_color.0;
        let text_bgr = [text_color[2], text_color[1], text_color[0]];
        let text_x0 = t.padding;
        let text_y0 = (t.search_bar_height - font::GLYPH_HEIGHT * SEARCH_FONT_SCALE) / 2;
...
fn blit_rounded(dst: &mut [u8], dst_w: i32, dst_h: i32, src: &RgbaImage, x0: i32, y0: i32, radius: i32, dim: f64) {
    ...
    for sy in 0..sh {
        ...
        for sx in 0..sw {
            ...
            if corner_mask(sx, sy, sw, sh, radius) {
                continue; // outside the rounded rect, leave background showing
            }
            let sp = src.get_pixel(sx as u32, sy as u32).0;
            let a = ((sp[3] as f64) * dim).round().clamp(0.0, 255.0) as u32;
            if a == 0 {
                continue;
            }
            let di = ((dy * dst_w + dx) * 4) as usize;
            for (c_dst, c_src) in [(0usize, 2usize), (1, 1), (2, 0)] {
                let bg = dst[di + c_dst] as u32;
                let fg = sp[c_src] as u32;
                dst[di + c_dst] = ((fg * a + bg * (255 - a)) / 255) as u8;
            }
            dst[di + 3] = dst[di + 3].max(a.min(255) as u8);
        }
    }
}
...
fn draw_ring(dst: &mut [u8], dst_w: i32, dst_h: i32, x0: i32, y0: i32, size: i32, radius: i32, width: i32, rgba: [u8; 4]) {
    ...
    for ly in -width..size + width {
        ...
        for lx in -width..size + width {
            ...
            let outside_outer = corner_mask(lx + width, ly + width, size + 2 * width, size + 2 * width, outer_r);
            if outside_outer {
                continue;
            }
            let inside_icon = lx >= 0 && lx < size && ly >= 0 && ly < size && !corner_mask(lx, ly, size, size, radius);
            if inside_icon {
            ...
            if a == 255 {
                ...
                for (c_dst, c_src) in [(0usize, 0usize), (1, 1), (2, 2)] {
                    let bg = dst[di + c_dst] as u32;
                    let fg = bgr[c_src] as u32;
                    dst[di + c_dst] = ((fg * a + bg * (255 - a)) / 255) as u8;
                }
            }
        }
...
/// True if (x, y) within a `w`x`h` box falls outside a `radius`-rounded
/// corner and should be left transparent.
fn corner_mask(x: i32, y: i32, w: i32, h: i32, radius: i32) -> bool {
    if radius <= 0 {
        return false;
    }
    let (cx, cy) = match (x < radius, y < radius, x >= w - radius, y >= h - radius) {
        (true, true, _, _) => (radius, radius),
        (_, true, true, _) => (w - radius, radius),
        (true, _, _, true) => (radius, h - radius),
        (_, _, true, true) => (w - radius, h - radius),
        _ => return false,
    };
    let dx = x - cx;
    let dy = y - cy;
